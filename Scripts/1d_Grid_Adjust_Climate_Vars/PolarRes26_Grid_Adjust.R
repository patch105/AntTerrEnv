...
#
# SEA ICE is handled entirely separately, by Step3b_SeaIce_GridAdjust.R --
# this script excludes every "Sea_Ice_Concentration" file from its job table
# so the two scripts never both touch the same file.
#
# Deliberately does NOT hard-code the list of variables/periods/products --
# it just finds every non-sea-ice .tif file Script 2 actually produced, for
# every model, and processes each one. That way this script automatically
# covers whatever Script 2 currently outputs (including anything added,
# renamed, or removed there later) without the two scripts needing to be
...
# ---- 2. Build the job table: every non-sea-ice .tif Script 2 produced -----------
# Sea-ice climatology files are excluded here -- they're handled entirely by
# Step3b_SeaIce_GridAdjust.R instead, so the two scripts never double-process
# the same file.
...
job_table <- map_dfr(models, function(model) {
  ...
  files <- files[!grepl("Sea_Ice_Concentration", basename(files), fixed = TRUE)]
  if (length(files) == 0) return(tibble())
  tibble(model = model, input_path = files, filename = basename(files))
})

if (nrow(job_table) == 0) {
  stop("No non-sea-ice .tif files found under ", input_base,
       " -- has Script 2 been run yet? (Sea-ice files are excluded here on ",
       "purpose -- see Step3b_SeaIce_GridAdjust.R.)")
}
...
# only for FUTURE-period files.
expected_outputs <- c(coastline_path, icefree_path)
if (is_future) expected_outputs <- c(expected_outputs, icefree_futurepath)
...
# ---- 5a. COASTLINE / ICEFREE / ICEFREE_FUTURE ------------------------------------
# ICEFREE_FUTURE is only written for FUTURE-period files and is saved as its
# own, separate output alongside COASTLINE/ICEFREE.
